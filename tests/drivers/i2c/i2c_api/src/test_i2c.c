...
static int test_gy271(void)
{
	...
	/* 6. verify i2c_read_string example */
	{
		struct i2c_msg msg[2];
		uint8_t offset = 0x01;
		uint8_t block_length;

		/* Step 1: read string length first */
		msg[0].buf = (uint8_t *)&offset; /* register offset */
		msg[0].len = 1;
		msg[0].flags = I2C_MSG_WRITE;
		/* string length (no I2C_MSG_STOP flag) */
		msg[1].buf = (uint8_t *)&block_length;
		msg[1].len = 1;
		msg[1].flags = I2C_MSG_RESTART | I2C_MSG_READ;
		/* start first transaction */
		i2c_transfer(i2c_dev, msg, 2, addr);

		/* Step 2: read string by length */
		msg[0].buf = &datas[0];
		msg[0].len = block_length;
		msg[0].flags = I2C_MSG_READ | I2C_MSG_STOP;
		i2c_transfer(i2c_dev, msg, 1, addr);

		TC_PRINT("i2c_read_string() test: reg %02x, len %02x: "
			"data %02x %02x %02x %02x\n",
			offset, block_length, datas[0], datas[1],
						datas[2], datas[3]);
